    ...
    function autocomplete(inp, arr) {
        ...
        for (i = 0; i < arr.length; i++) {
            /*check if the item starts with the same letters as the text field value:*/
                ...
                var title = '';
                for (var headvalue in arr[i].exercise_head.headvalues) {
                    if(headvalue.language_id == <?php echo $active_language;?>){
                        title += headvalue.value;
                    }
                }
                title += ' '+arr[i].exercise_difficulty+' ('+arr[i].exercise_difficulty+' questions)';
                
                b.innerHTML = '<label>'+title+'</label><label><b>'+arr[i].article_title+'</b></label>';
